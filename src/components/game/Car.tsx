...
export function Car() {
  ...
  const damageState = useGameStore((state) => state.damageState);
  ...
  useFrame((state) => {
    ...
    const { forward, backward, left, right, brake, action, boost } = controls;
    
    // Performance Multipliers
    let engineMultiplier = 1;
    let steeringMultiplier = 1;

    if (damageState === 'light') engineMultiplier = 0.9;
    if (damageState === 'heavy') engineMultiplier = 0.7;
    if (damageState === 'critical') {
      engineMultiplier = 0.4;
      steeringMultiplier = 0.6;
      // Chance of engine sputter
      if (Math.random() < 0.05) engineMultiplier = 0;
    }
    if (damageState === 'destroyed') {
      engineMultiplier = 0;
      steeringMultiplier = 0.1; // Seized
    }

    // Power-up activation
    if (action && currentPowerUp && chassisBody.current && damageState !== 'destroyed') {
       const position = new THREE.Vector3();
       const quaternion = new THREE.Quaternion();
       chassisBody.current.getWorldPosition(position);
       chassisBody.current.getWorldQuaternion(quaternion);
    ...
    const baseEngineForce = forward ? 1500 : backward ? -800 : 0;
    const engineForce = (boost && forward ? baseEngineForce * 2 : baseEngineForce) * engineMultiplier;
    const steeringValue = (left ? 0.5 : right ? -0.5 : 0) * steeringMultiplier;
  ...
  // Damage visual emitters
  useFrame((state) => {
    if (damageState === 'none' || status !== 'playing') return;
    ...
    if (chassisBody.current) {
      ...
      if (damageState === 'heavy' && state.clock.elapsedTime % 0.5 < 0.02) {
        addEffect('hit', posArr); // Small sparks
      }
      if (damageState === 'critical' && state.clock.elapsedTime % 0.3 < 0.02) {
        addEffect('nitro-blast', posArr); // Small fire bursts
      }
      if (damageState === 'destroyed' && state.clock.elapsedTime % 0.1 < 0.02) {
        addEffect('hit', posArr);
        addEffect('nitro-blast', posArr);
      }
    }
  });
